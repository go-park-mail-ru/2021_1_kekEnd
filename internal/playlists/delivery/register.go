...
func RegisterHttpEndpoints(router *gin.Engine, playlistsUC playlists.UseCase, authMiddleware middleware.Auth, Log *logger.Logger) {
	...
	router.POST("/playlists/movie", authMiddleware.CheckAuth(), handler.AddMovieToPlaylist)
	router.DELETE("/playlists/movie", authMiddleware.CheckAuth(), handler.DeleteMovieFromPlaylist)

	router.POST("/playlists/user", authMiddleware.CheckAuth(), handler.AddUserToPlaylist)
	router.DELETE("/playlists/user", authMiddleware.CheckAuth(), handler.DeleteUserFromPlaylist)
}
